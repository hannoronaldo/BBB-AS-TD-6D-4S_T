...
def set_pwm_values_at_bbb():
    ...
    #hold this cade block here may for later use
    #scv_flw = round(duty_span/2 + offset_steering[0], 3) # set the steering to 90° scv -> servo_center_value
    #scv_rlw = round(duty_span/2 + offset_steering[1], 3) # set the steering to 90° scv -> servo_center_value
    ...
    if rover_Drive['steering_direction'] == 'right':
        steering_fl_dc = round(((90 + rover_Drive['FLW'][0] - 0) * (duty_max-duty_min)) / (180 + duty_min) + offset_steering[0], 2)
        steering_rl_dc = round(((90 - rover_Drive['RLW'][0] - 0) * (duty_max-duty_min)) / (180 + duty_min) + offset_steering[1], 2)
        steering_fr_dc = round(((90 + rover_Drive['FRW'][0] - 0) * (duty_max-duty_min)) / (180 + duty_min) + offset_steering[2], 2)
        steering_rr_dc = round(((90 - rover_Drive['RRW'][0] - 0) * (duty_max-duty_min)) / (180 + duty_min) + offset_steering[3], 2)
...
def ackermann_steering(inp_tuple):
    ...
    :param distance:  int value of cm to travel
    :return: none
    """
    # some init
...
ackermann_steering((10, -25, 20))  # eg.  +/-=indicate left or right, 15 = 15 degree, 100 = speed in duty cycle percent, 20=distance in cm
